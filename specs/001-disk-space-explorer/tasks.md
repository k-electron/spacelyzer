...
- [ ] T133 Give scanned nodes an integer identity assigned during traversal and key `FilterResult` by it instead of by path, in `Spacelyzer/Analysis/FilterEvaluator.swift` and `CategoryAnalyzer.swift` (SC-009) — a filter change currently allocates and hashes a path string at every one of a million nodes, twice, which is where 1.55 seconds of a 200 ms budget goes. Merging the breakdown into the evaluator's single walk halves what is left. Whether the two together reach 200 ms is unmeasured, and `SpacelyzerTests/ScaleTests.swift` is where to find out. Raised by T124
